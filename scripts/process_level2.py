...
async def main(loop):
    ...
    try:
        found_keys = nexrad_fetcher.find_level2_keys(
            None, site, start_time_utc, end_time_utc
        )
    except Exception as e:
    ...
    if not found_keys:
        ...
        try:
            log.info("Running cleanup for old processed files...")
            deleted_count = drive_service.delete_old_files(
                config.PREFIX_PLOTS_L2,
                config.CLEANUP_WINDOW_MINUTES,
            )
            log.info(f"Cleanup finished. Deleted {deleted_count} old files.")
        except Exception as e:
    ...
    # 4. Get Existing Processed Files List
    log.info(f"Retrieving existing processed file list for L{level}/{product}...")
    try:
        existing_files_dict = metadata_service.get_file_list(level, product)
    ...
    if new_files_metadata:
        ...
        try:
            ...
            if update_success:
                ...
                flag_success = metadata_service.set_update_flag(product)
                if flag_success:
    ...
    # 11. Cleanup Old Processed Files
    try:
        log.info("Running cleanup for old processed files...")
        deleted_count = drive_service.delete_old_files(
            config.PREFIX_PLOTS_L2,
